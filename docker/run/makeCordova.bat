...
docker run -d   -p 4200:4200 -p 49153:49153 --mount type=bind,source=/E/ignatandrei/netCoreAlphabet/angularCards,target=/usr/app --name angularCards_container angular_modify_image
REM TODO see container id
REM docker exec angularCards_container npm i
REM docker exec -it angularCards_container  ng serve  --host 0.0.0.0 --poll 10
REM docker exec -it angularCards_container ng add @angular/material

REM TESTING

REM create image
rem docker build --tag angular_test --file testAng.txt .
...
REM testing
REM docker exec -it angular_test_container ng test --poll 10